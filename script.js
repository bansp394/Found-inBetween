...
const BUILDINGS = {
  ...
  '80 Fifth Ave': {
    ...
    items: [
      ...
      { id: 30, type: 'lost',  title: 'Scarf Lost',   loc: '80 Fifth Ave – Coat Rack',    time: '2 days ago',  what: 'Striped wool scarf, red and cream colored.', where: 'Coat rack near the main entrance.', when: 'Monday', email: 'anon@newschool.edu' },
    ]
  },
  ...
  'Sheila C. Johnson Design Center': {
    ...
    items: [
      ...
      { id: 58, type: 'found', title: 'Jacket Found',        loc: 'SCJDC – Lecture Hall',  time: '3 days ago',  what: 'Tan trench coat, size M, left on a seat after an event.', where: 'Lecture hall seating, row C.', when: 'Last Thursday', email: 'anon@newschool.edu' },
    ]
  },
  ...
  'Woodshop': {
    ...
    items: [
      ...
      { id: 47, type: 'lost',  title: 'Project Model Lost',   loc: 'Woodshop – Storage Shelf', time: 'Yesterday',   what: 'Small wooden architectural model, labeled with initials J.K.', where: 'Storage shelf along the back wall.', when: 'Yesterday', email: 'jk@newschool.edu' },
    ]
  },
...
function renderActivity(items) {
  ...
  list.innerHTML = items.map(item => `
    <div class="activity-item" onclick="goItem(${item.id})">
      ...
      <div class="activity-meta"><span>${item.time}</span><span class="activity-chevron">›</span></div>
    </div>
  `).join('');
}
...
if (currentItem.image) {
  imgBox.innerHTML = `<img src="${currentItem.image}" alt="item image">`;
} else {
  imgBox.innerHTML = '<span class="detail-image-placeholder">📷 No image provided</span>';
}
...
function sendDetailMsg(e) {
  if (e.key === 'Enter') sendDetailMsgClick();
}
